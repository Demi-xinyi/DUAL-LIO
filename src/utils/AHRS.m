    ...
    methods (Access = public)
        ...
        function obj = UpdateIMU(obj, Gyroscope, Accelerometer)
            ...
            if(norm(Accelerometer) == 0)                                          	% handle NaN
                ...
                Accelerometer = Accelerometer / norm(Accelerometer);                % normalise measurement；norm向量范数和矩阵范数
            end
...
%             end
            ...
            pDot = 0.5 * obj.quaternProd(obj.q, [0 Ref(1) Ref(2) Ref(3)]);          % compute rate of change of quaternion四元数微分
...
%             disp(obj.q);
        ...
        function obj = Updateyawl(obj,Gyroscope, mocap_euler)
...
%             end
            ...
            euler = quatern2euler(obj.Quaternion);
            error = euler(3) - mocap_euler(3);
            obj.IntError_yawl = obj.IntError_yawl + error;
            euler(3) = euler(3) - (obj.Kp*error + obj.Ki*obj.IntError_yawl);%'- (error * Kp_yawl + error_i *Ki_yawl);
            obj.Quaternion = euler2quatern(euler);
        end
